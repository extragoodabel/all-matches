...
export function AllMatchesLogo({ 
    ...
    extrusionLayers.push(
      <g key={`extrusion-${i}`} transform={`translate(${x}, ${y})`}>
        {MATCHES_PATHS.map((d, idx) => (
          <path key={idx} fill={matchesShadowColor} stroke={!withOutline ? matchesShadowColor : "none"} strokeWidth={!withOutline ? 2 : 0} strokeLinejoin="round" d={d} />
        ))}
      </g>
    );
  }
  ...
  return (
    ...
    >
      <g>
        <path fill={heartColor} d="M208.14,35.57c-28.88-22.06-71.9-14.48-96.1,16.95-28.88-22.06-71.9-14.48-96.1,16.95-24.2,31.42-20.41,74.78,8.47,96.84l99.02,75.65q5.56,4.25,9.83-1.29l83.36-108.25c24.2-31.42,20.41-74.78-8.47-96.84Z"/>
